...
`RangeFilter` compone `DualRangeSlider`, un adaptador propio sobre `@react-native-assets/slider`, y conserva debajo dos steppers compactos `[− | valor + unidad | +]` para Mínimo y Máximo. Slider, edición directa y botones leen el mismo `NumericRange`: no duplican estado ni cambian reducer, filtrado o persistencia. La barra ofrece un track único con tramo activo y dos tiradores de tamaño explícito; impide el cruce durante el gesto. Los campos mantienen la edición exacta y son el fallback accesible si una plataforma o lector de pantalla no resuelve bien los tiradores. Cada valor se limita contra el extremo opuesto antes del callback, mientras el reducer continúa como defensa final. Un campo vacío se restaura al salir. La dependencia queda aislada en `src/features/filters/components/DualRangeSlider.tsx` para poder sustituirla sin afectar al dominio; ver [ADR-011](decisiones/ADR-011-dual-range-slider.md).
