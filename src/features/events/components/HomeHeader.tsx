...
import { breakpoints, colors, motion, sizes, spacing, typography } from '@/theme';
...
export function HomeHeader({ query, onQueryChange }: { query: string; onQueryChange: (value: string) => void }) {
  ...
  const searchWidth = Math.max(0, contentWidth - spacing.lg * 2 - sizes.touchTarget - spacing.sm);
...
const styles = StyleSheet.create({
  ...
  searchSlot: {
    position: 'absolute',
    right: spacing.lg + sizes.touchTarget + spacing.sm,
    overflow: 'hidden',
  },
