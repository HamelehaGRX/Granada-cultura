/* CONFIGURACIÓN Y DATOS INICIALES
   Define claves locales, categorías, estados y eventos ficticios. Estos bloques se podrán
   sustituir más adelante por fuentes reales sin mezclar la interfaz con el origen de datos. */
...
/* ESTADO DE LA INTERFAZ
   Mantiene eventos, preferencias, rango temporal, vista y selección activa durante la sesión. */
...
/* PERSISTENCIA LOCAL
   Lee y guarda eventos y preferencias en el navegador; actualmente no existe backend. */
function loadEvents() {
  try {
...
/* UTILIDADES DE DATOS Y FECHAS
   Protegen el HTML generado y normalizan fechas, rangos y etiquetas de precio. */
...
/* FILTRADO Y REPRESENTACIÓN
   Aplica los controles activos y construye las tarjetas que aparecen en las listas. */
...
/* RENDERIZADO PRINCIPAL
   Actualiza filtros, agenda, favoritos y el aviso de demostración a partir del estado. */
...
/* ACCIONES SOBRE EVENTOS
   Gestiona favoritos, estados, apertura del detalle, edición, alta y eliminación. */
...
/* CALENDARIO MENSUAL
   Genera la cuadrícula de seis semanas y enlaza cada evento con su detalle. */
...
/* INTERCAMBIO DE DATOS
   Exporta e importa copias JSON y crea archivos ICS para calendarios externos. */
function exportData() {
  const payload = {
    app: "Granada Cultura",
    version: 1,
...
/* NAVEGACIÓN, AVISOS Y TEMA
   Cambia paneles, sincroniza botones, muestra confirmaciones y aplica el tema visual. */
...
/* ENLACE DE CONTROLES
   Registra todos los eventos de interacción una sola vez al iniciar la aplicación. */
function initEvents() {
...
/* INSTALACIÓN PWA Y ARRANQUE
   Gestiona la instalación, registra el service worker y ejecuta el render inicial. */
